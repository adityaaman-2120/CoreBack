...
function tenant(ownerId: string) {
  ...
  return database().collection("coreback_workspaces").doc(key(ownerId));
}
...
async function unlocked(ownerId: string, tx?: Transaction) {
  ...
  return snap.data() ?? { coreCount: 0, policyCount: 0 };
}
...
export async function insertCores(ownerId: string, cores: Core[]) {
  return database().runTransaction(async (tx) => {
    const meta = await unlocked(ownerId, tx);
    ...
    const policies = await tx.getAll(
      ...policyIds.map((id) => collection(ownerId, "policies").doc(recordId(id))),
    );
    if (policies.some((d) => !d.exists))
      throw new ApiError(422, "Select a supplier policy from this workspace.");
    const allocations = cores.flatMap((c) =>
      c.state.credits
    ...
    if (allocations.length) {
      const prior = await tx.getAll(...allocations.map((a) => a.ref));
...
export async function quota(quotaKey: string, limit: number) {
  const ref = database().collection("coreback_quotas").doc(key(quotaKey));
  await database().runTransaction(async (tx) => {
