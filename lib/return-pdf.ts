...
export function createReturnPdf(core: Core) {
  ...
  const newPage = () => {
    ...
    pdf.setFontSize(10);
    pdf.text("COREBACK / RETURN DETAILS", margin, 16);
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    pdf.setFontSize(8);
    pdf.text(
      `CoreBack | ${core.invoice} | Shop-entered evidence | ${i}/${pages}`,
      margin,
      287,
    );
  }
...
export async function downloadReturnPdf(core: Core) {
  createReturnPdf(core).save(`CoreBack-Return-${core.invoice}.pdf`);
}
