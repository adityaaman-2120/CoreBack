  ...
  try {
    ...
    if (path[0] === "voice" && path[1] === "token") {
      ...
      try {
        ...
        if (!upstream.ok) {
          console.error("Voice token request failed", upstream.status);
          throw new ApiError(
            502,
            `The voice service could not start the session (${upstream.status}). Check the voice provider account access and credit balance.`,
          );
        }
        ...
        if (!tokenData.token)
          throw new ApiError(
            502,
            "The voice service did not return a session token.",
          );
      ...
      if (path[2] === "transcript") {
        const d = z
          .object({
            ...
            speaker: z.enum(["Technician", "CoreBack"]),
