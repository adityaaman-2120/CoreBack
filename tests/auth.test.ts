...
beforeEach(() => {
  vi.clearAllMocks();
  vi.stubEnv("APP_ORIGINS", "https://coreback.test");
...
describe("Firebase session security", () => {
  ...
  it("rejects malformed and oversized token requests without calling Firebase", async () => {
    ...
    expect(
      (
        await session(
          request({}, "https://coreback.test", { "Content-Length": "13000" }),
        )
      ).status,
    ).toBe(401);
    expect(mocks.verifyIdToken).not.toHaveBeenCalled();
  ...
  it("fails closed for expired, revoked, or invalid session cookies", async () => {
    ...
    expect(await getUser()).toBeNull();
    await expect(requireUser("/workspace")).rejects.toThrow(
