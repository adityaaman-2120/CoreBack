...
flowchart LR
    ...
    App -->|Admin SDK transactions| DB[(Firestore)]
...
The server accesses the Firestore database selected by `FIRESTORE_DATABASE_ID` (default `(default)`) through its runtime service account. Firestore browser rules deny all direct reads and writes. Server-side tenant checks remain essential because the Admin SDK uses IAM permissions rather than browser rules.
...
No other AI provider is in the production voice path.
