...
import type { Core, CoreAction } from "../lib/domain";
import { samplePolicy, sampleCores } from "./fixtures";
...
async function makeCore(line = "1") {
  const p = (await request("policies", samplePolicy())).data.policy;
...
beforeAll(() => {
  ...
  process.env.GOOGLE_CLOUD_PROJECT = "demo-coreback-test";
...
describe.skipIf(!process.env.FIRESTORE_EMULATOR_HOST)(
  ...
  () => {
    it("rejects anonymous access and cross-origin or non-JSON mutations", async () => {
      ...
      expect(
        (await request("cores", {}, { Origin: "https://other.test" })).status,
      ).toBe(403);
      expect(
        (await request("cores", {}, { "Content-Type": "text/plain" })).status,
      ).toBe(415);
    });
    ...
    it("serializes concurrent purchase imports", async () => {
      const p = (await request("policies", samplePolicy())).data.policy;
      const input = {
        invoice: "INV-RACE",
        purchaseLine: "1",
    ...
    it("blocks writes during interrupted deletion and safely resumes a stale lease", async () => {
      await returned();
      const ref = database()
        .collection("coreback_workspaces")
      ...
      expect((await request("policies", samplePolicy())).status).toBe(409);
      expect(
        (
          await request("workspace-delete", {
            confirmation: "DELETE MY WORKSPACE",
      ...
      expect(await workspace()).toHaveLength(0);
      expect((await request("export")).data.events).toHaveLength(0);
      expect((await request("policies", samplePolicy())).status).toBe(200);
    });
    ...
    it("rejects invented quotes and agent speech as technician evidence", async () => {
      ...
      await f.transcript(Object.values(f.quotes).join(". "), "CoreBack");
      expect((await f.tool()).status).toBe(422);
    ...
    it("lists purchases in stable return urgency order", async () => {
      const policy = (await request("policies", samplePolicy())).data.policy;
      await insertCores(
        identity.id!,
        sampleCores().map((c) => ({
          ...c,
