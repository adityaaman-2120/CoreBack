...
import { ArrowRight, ShieldCheck, Mic2, BookOpenCheck } from "lucide-react";
...
function explain(code: string) {
  const messages: Record<string, string> = {
    ...
    "auth/admin-restricted-operation":
      "Guest access is not enabled for this deployment. Create an account instead.",
    "auth/operation-not-allowed":
      "This sign-in method is not enabled for this deployment.",
    "auth/user-disabled": "This account has been disabled.",
    "auth/unauthorized-domain":
      "This site address is not authorized for sign-in. Ask the administrator to add it in Firebase Authentication.",
    "auth/requests-from-referer-blocked":
      "Sign-in is blocked for this site address. Ask the administrator to allow it for the Firebase API key.",
  };
  ...
  return (
    <main className="auth-page">
      <section className="auth-story">
        <Brand light />
        <span className="auth-eyebrow">VOICE-FIRST CORE-DEPOSIT RECOVERY</span>
        <h1>
          Your old parts.
          <br />
          Your money back.
        ...
        <div className="auth-value">
          <BookOpenCheck />
          <div>
            <strong>Start empty, stay in control</strong>
            <span>No sample data. Add your own policies and purchases.</span>
          </div>
        </div>
        <small>CoreBack Voice · © {SITE.year} {SITE.name}</small>
      </section>
      <section className="auth-panel">
        ...
        <div className="auth-card">
          <span className="auth-eyebrow">WELCOME TO {SITE.name.toUpperCase()}</span>
          <h2>
            {mode === "signup"
          ...
          <p>
            {mode === "signup"
              ? "Start with a blank, private desk. No email verification needed."
              : mode === "reset"
